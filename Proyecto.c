...
    printf("Ingresa los votos en tríos (Cancion1 Cancion2 Cancion3) del 1 al 10\n");
    printf("Ingresa '-1' en el primer valor y cualquier valor en las otras 2 opciones para terminar \n\n");
...
        printf("Oyente %d: ", *total_oyentes + 1);
...
        // Validación: Canciones deben estar entre 1 y NUM_CANCIONES
        if (c1 < 1 || c1 > NUM_CANCIONES || c2 < 1 || c2 > NUM_CANCIONES || c3 < 1 || c3 > NUM_CANCIONES) {
            printf("  -> Voto invalido. Las canciones deben ser del 1 al %d. Intenta de nuevo.\n", NUM_CANCIONES);
            continue;
        }

        // Validación: No permitir votar por la misma canción 3 veces
        if (c1 == c2 || c1 == c3 || c2 == c3) {
            printf("  -> Voto invalido. Una cancion no puede repetirse en el mismo trio. Intenta de nuevo.\n", NUM_CANCIONES);
            continue;
        }
...
void calcularResultados(int votos[MAX_OYENTES][3], int total_oyentes, int *ganador1, int *ganador2) {
    // Arreglo de tamaño 11 para usar índices del 1 al 10
    int puntajesCanciones[NUM_CANCIONES + 1] = {0};
...
            // Desplazar el 1er lugar al 2do lugar
            max2 = max1;
            *ganador2 = *ganador1;
            // Asignar el nuevo 1er lugar
...
        }
        
        // 10 puntos suplementarios si acertó ambas canciones 
...
        // Verificar quién tiene el puntaje más alto (>= para permitir empate)
