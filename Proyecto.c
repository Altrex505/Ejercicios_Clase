...
int main() {
    ...
    printf("Ingresa los votos en tríos (Cancion1 Cancion2 Cancion3)\n");
    printf("Ingresa '-1' en el primer valor y cualquier opción en los otros dos valores para terminar \n\n");
...
void calcularResultados(int votos[MAX_OYENTES][3], int total_oyentes, int *ganador1, int *ganador2) {
    int puntajesCanciones[NUM_CANCIONES + 1] = {0}; // Inicializar en 0

    // Sumar los puntos de cada canción
    for (int i = 0; i < total_oyentes; i++) {
        // Se suma el puntaje según el orden de preferencia (3, 2, 1)
...
void calcularGanadorPremio(int votos[MAX_OYENTES][3], int total_oyentes, int cancion1, int cancion2) {
    ...
    printf("\n GANADOR DEL PREMIO \n El oyente numero %d con %d puntos\n", oyenteGanador, maxPuntos);
}
